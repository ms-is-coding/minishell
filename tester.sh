...
test_cmd() {
	...
	timeout 3s ./minishell -c "$cmd" 1>/tmp/minishell_stdout.txt 2>/tmp/minishell_stderr.txt
	local minishell_exit=$?

	timeout 3s bash -c "$cmd" 1>/tmp/bash_stdout.txt 2>/tmp/bash_stderr.txt
	local bash_exit=$?


	if [ "$minishell_exit" -ne "$bash_exit" ]; then
		echo -e $RED"✗"$RESET$BOLD" $test_name: Exit codes differ (minishell: $minishell_exit, bash: $bash_exit)"$RESET
		FAILED=$((FAILED + 1))
	elif ! diff /tmp/minishell_stdout.txt /tmp/bash_stdout.txt >/dev/null 2>&1; then
		echo -e $RED"✗"$RESET$BOLD" $test_name: stdout differs"$RESET
		diff --color --side-by-side /tmp/bash_stdout.txt /tmp/minishell_stdout.txt
		echo
		FAILED=$((FAILED + 1))
	elif ! diff /tmp/minishell_stderr.txt /tmp/bash_stderr.txt >/dev/null 2>&1; then
		echo -e $YELLOW"⚠"$RESET$BOLD" $test_name: stderr differs"$RESET
		PASSED=$((PASSED + 1))
	else
		echo -e $GREEN"✓"$RESET$BOLD" $test_name"$RESET
		PASSED=$((PASSED + 1))
	fi

	rm -f /tmp/minishell_stdout.txt /tmp/bash_stdout.txt
	rm -f /tmp/minishell_stderr.txt /tmp/bash_stderr.txt
}
...
run_all_tests() {
	...
	test_cmd "Magic cd" "mkdir -p a/b; cd a/b; rm -r ../../a; cd ..; cd ..; pwd"

	test_cmd "env" "env | sort"
	...
	test_cmd "Subshells" "(echo hello | echo world) | (cat | rev)"
